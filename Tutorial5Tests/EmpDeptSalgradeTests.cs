...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo)
            .Select(eGroup => new { DeptNo = eGroup.Key, Count = eGroup.Count() })
            .ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(_ => grades, (e, s) => new { e, s })
            .Where(row => row.e.Sal >= row.s.Losal && row.e.Sal <= row.s.Hisal)
            .Select(row => new {row.e.EName, row.s.Grade})
            .ToList(); 
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo)
            .Select(eGroup => new {DeptNo = eGroup.Key, AvgSal = eGroup.Average(e => e.Sal)}); 
